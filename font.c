...
Status Font_Load( Font* font, char* fontTexturePath, FontData* fontData ) {
    ...
    Status loadStatus = Image_Load( 
        &font->fontImage, fontTexturePath, FONTVRAM_X, FONTVRAM_Y, FONTCLUT_X, FONTCLUT_Y 
    );
...
int Renderer_DrawText( Renderer* renderer, Font* font, char* text, int x, int y, Color tint ) {
    ...
    int pixelPerWord = Is4Bit( &font->fontImage ) ? 4 : Is8Bit( &font->fontImage ) ? 2 : 1;
    int wordPerTPage = Is4Bit( &font->fontImage ) ? 64 : Is8Bit( &font->fontImage ) ? 128 : 256;

    //Used for bitshifting (optimization).
    int bitsPerTPage = Is4Bit( &font->fontImage ) ? 6 : Is8Bit( &font->fontImage ) ? 7 : 8;
    ...
    int tPageX = ( font->fontImage.tim.prect->x >> bitsPerTPage ) << bitsPerTPage;
    int tPageY = ( font->fontImage.tim.prect->y >> 8 ) << 8;
    ...
    for( int i = 0; i < stringLength; i++ ) {
        ...
        CharData* currentCharactersData = &font->fontData->charData[ currentCharacterIndex ];
