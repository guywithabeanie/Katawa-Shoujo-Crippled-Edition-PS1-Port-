...
int SnowSystem_Update(SnowSystem *snowSystem) {
  for (int i = 0; i < SNOW_COUNT; i++) {
    ...
    if (snowFlake->x > itoF(SCREEN_WIDTH))
      snowFlake->x = itoF(-snowSystem->snowSprite->prect.w);
    if (snowFlake->y > itoF(SCREEN_HEIGHT))
      snowFlake->y = itoF(-snowSystem->snowSprite->prect.h);
  }
  return 0;
}
...
int Renderer_DrawSnow(Renderer *renderer, SnowSystem *snowSystem) {
  for (int i = 0; i < SNOW_COUNT; i++) {
    SnowFlake *snowFlake = &snowSystem->snow[i];
    Renderer_DrawImage(renderer, snowSystem->snowSprite, Ftoi(snowFlake->x),
                       Ftoi(snowFlake->y), DEFAULT, QuarterAdditive);
  }
  return 0;
}
